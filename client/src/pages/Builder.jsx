...
function Builder({user, setUser}){
    ...
    const saveAssistant = async()=>{
        ...
        try {
            ...
            toast("Failed to save assistant, error occured")
            console.log(error)
            setLoading(false)
        }
    }
    ...
    const remainingMessages = Math.max(0,
        (user?.requestLimit || 0) - (user?.totalMessages || 0)
    );


    const remainingDays= user?.proExpiresAt?Math.max(
        0,Math.ceil(
    ...
    <script src="${CLIENT_URL}/assistant.js" data-user-id="${user?._id}"> </script>
                            ...
                            <p className="mt-3 text-sm font-medium text-[#081028] mb-3">
                                Embed Code
                            </p>
                        </div>
                ...
                {editAssistant && <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                        ...
                        <div className="mt-6">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {THEMES.map((item)=>(
                                    <button key={item} onClick={()=>setTheme(item)} className={`py-3 rounded-2xl border-2 capitalize ${theme===item? "border-purple-500 bg-purple-100 text-purple-700" : "border-gray-200"}`}>{item}</button>
                                ))}
                        ...
                        <div className="mt-6">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {TONES.map((item)=>(
                                    <button key={item} onClick={()=>setTone(item)} className={`py-3 rounded-2xl border-2 capitalize ${tone===item? "border-purple-500 bg-purple-100 text-purple-700" : "border-gray-200"}`}>{item}</button>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
                        <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
                            ...
                            <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-gradient-to-r from-purple-500 to-emerald-500 text-white text-sm font-medium rounded-xl hover:scale-[1.02] transition-all cursor-pointer">
                                Get API KEY
                            </a>
                        </div>
                        <input type="password" placeholder="AIza...." onChange={(e)=>setGeminiApiKey(e.target.value)} value={geminiApiKey} className="w-full border border-gray-200 rounded-2xl px-4 py-3" />
                    ...
                    <button onClick={saveAssistant} disabled={loading} className="w-full h-14 rounded-2xl text-white font-semibold bg-gradient-to-r from-purple-500 to-emerald-500">
                        {
                            loading?"Saving...":user.isSetupComplete?"Update Assitant" : "Save Assistant"
                        }
                    </button>
